...
static void test_open_close() {
    DiskManager* dm = NULL;
    DBStatus status = db_open(&dm, TEST_DB);
    if (status == DB_OK) {
...
static void test_allocate_write_read() {
    ...
    page_id_t page_id = db_allocate_page(dm);
    ...
    status = db_write_page(dm, page_id, &write_page);
    if (status == DB_OK) {
    ...
    status = db_read_page(dm, page_id, &read_page);
    if (status == DB_OK) {
...
static void test_multiple_pages() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        status = db_write_page(dm, pages[i], &page);
        if (status != DB_OK) {
    ...
    for (int i = 0; i < 5; i++) {
        Page page;
        memset(&page, 0, sizeof(Page));
        status = db_read_page(dm, pages[i], &page);
        if (status != DB_OK) {
...
static void test_free_page() {
    ...
    status = db_free_page(dm, page_id);
    if (status == DB_OK) {
    ...
    page_id_t new_page_id = db_allocate_page(dm);
